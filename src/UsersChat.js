...
function UserChat({ openchat, setOpenChat }) {
  ...
  return (
    <div className="md:h-80% md:w-97 text-area pt-28 overflow-y-scroll    md:rounded-xl md:right-6 h-full z-30 w-full dropdown shadow-xl fixed bottom-0  md:bottom-6">
      ...
      <section className="fixed md:block  md:w-97 pr-4  pl-4  z-40 bottom-4 w-full md:bottom-20">
        <form>
          <textarea
            value={text}
            rows="1"
            onChange={handleChange}
            className=" pl-4 pr-14 text-area overflow-y-scroll block h-auto max-h-32 w-90%  placeholder:text-base placeholder:font-normal text-base font-medium  outline-none rounded-2xl py-1.5 bg-white"
        ...
        <div className="h-10 grid place-items-center cursor-pointer fixed right-2 md:right-8 bottom-4 md:bottom-20 rounded-full text-white bg-blue-600 w-10">
          <svg
            fill="#fff"
            width="20px"
            height="20px"
      ...
      <div className="h-300  text-white mx-6 ">
        ...
        <div className="text-white">haaaa</div>
      </div>
    </div>
  );
}
